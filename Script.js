...
function favourite(element){
	...
	for(let i = 0; i< currentMovieStack.length; i++){
		if(currentMovieStack[i].id == id){
			...
			favouriteMoviesImran.unshift(currentMovieStack[i]);
			localStorage.setItem("favouriteMoviesImran", JSON.stringify(favouriteMoviesImran));
...
goToFavouriteButton.addEventListener('click', ()=>{
	let favouriteMoviesImran = JSON.parse(localStorage.getItem("favouriteMoviesImran"));
	if(favouriteMoviesImran == null || favouriteMoviesImran.length < 1){
		showAlert("you have not added any movie to favourite");
		return;
	}

	currentMovieStack = favouriteMoviesImran;
	renderList("remove");
})
...
function remove(element){
	...
	for(let i = 0; i<favouriteMoviesImran.length; i++){
		if(favouriteMoviesImran[i].id == id){
			continue;
		}
		newFavouriteMovies.push(favouriteMoviesImran[i]);
	}
	
	localStorage.setItem("favouriteMoviesImran", JSON.stringify(newFavouriteMovies));
